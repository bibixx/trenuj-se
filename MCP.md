...
| `update_workouts`   | Update one or more workouts. Each item requires `workoutId`. All other fields optional. Set to `null` to clear. Supports partial success — valid updates apply even if others fail.                                                                     |
| `remove_workouts`   | Delete workouts. Required: `workoutIds` array.                                                                                                                                                                                                          |
| `get_workouts`      | Query workouts. Optional filters: `planId`, `dateFrom`, `dateTo`, `labelId`, `labelKey`, `status`, `limit` (default 50, max 200). Pass `compact: true` for id/date/title/status/label only — the full payload includes execution JSONB and notes.       |
...
| `hydrate_activities` | Bulk-pull detail/laps/streams from Strava for up to 10 activities per call, ahead of cross-session stream analysis. Required: `stravaIds` (1–10). Idempotent — already-hydrated ids report `already`.                                                                   |

**Hydration is automatic and inferred from the query.** Any Strava activity id **or workout UUID** appearing in the SQL gets its detail/laps/per-second streams pulled from Strava before the query runs (max 3 ids per call), and queries whose _results_ return `strava_id` rows (e.g. finding an activity by date) hydrate those and re-run once. A `-- hydrate: id1, id2` comment forces specific ids; `hydrate_activities` bulk-pulls up to 10 per call. `activities.stream_channels` records which stream columns actually contain data, so agents don't query all-NULL columns. The webhook keeps new activities flowing in and everything ever matched to a workout is pre-loaded; the only invisible data is old never-matched activities, which appear after being referenced by id once.
