...
import { distanceUnitFromPaceUnit, isPaceTimeUnit, paceTimeToSeconds } from "./workout-execution-pace";
import type {
...
/**
 * Builds an Apple `.workout` WorkoutPlan from a stored WorkoutExecution.
 * Pure (no DOM / Node globals), so it runs in both the browser and the Worker.
 */
export function buildWorkoutPlanFromExecution(referenceId: string, title: string, execution: WorkoutExecution): WorkoutPlan | null {
