...
- `execution` — structured machine-facing data (for workout builders, Apple Watch export). Includes block types: `warmup`, `cooldown`, `steady`, `rest`, `free`, `interval`, `repeat`, `strength`, `note`. Only populate if you have precise targets.
